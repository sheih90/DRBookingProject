...
class APIClient:
    ...
    def update_booking(self, booking_id, update_data):
        with allure.step(f"Updating booking {booking_id}"):
            url = f"{self.base_url}/{Endpoints.BOOKING_ENDPOINT}/{booking_id}"
            response = self.session.put(url,json=update_data, auth=HTTPBasicAuth(Users.USERNAME, Users.PASSWORD))
            response.raise_for_status()
        with allure.step('Checking status code'):
            assert response.status_code == 200, f"Expected status 200 but got {response.status_code}"
            return response.json()
    ...
    def partial_update_booking(self, booking_id, partial_data):
        with allure.step(f"Partial update booking {booking_id}"):
            url = f"{self.base_url}/{Endpoints.BOOKING_ENDPOINT}/{booking_id}"
            response = self.session.patch(url,json=partial_data, auth=HTTPBasicAuth(Users.USERNAME, Users.PASSWORD))
            response.raise_for_status()
        with allure.step('Checking status code'):
            assert response.status_code == 200, f"Expected status 200 but got {response.status_code}"
            return response.json()
